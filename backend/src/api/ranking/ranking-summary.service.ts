...
export class RankingSummaryService {
  constructor(private readonly rankingService: RankingService, private readonly backupService: BackupService) {}

  // userNetID 우선순위 큐 관리
  private userQueue: string[] = [];
  private userCache: { uid: string | null; timestamp: number } = { uid: null, timestamp: 0 };

  /**
   * userNetID 중 유효한 ID를 찾아 반환 (캐시/순환)
   */
  private async getValidUserId(userIds: string[], teamMode: number): Promise<string> {
    ...
    if (uid && now - timestamp < CACHE_TTL) {
      try {
        await this.rankingService.getRankingData({ userNetID: uid, teamMode, region: 'ES', rankingType: 1, champType: 0, rowCount: 100 });
        return uid;
      } catch {
    ...
    for (const id of userIds) {
      try {
        await this.rankingService.getRankingData({ userNetID: id, teamMode, region: 'ES', rankingType: 1, champType: 0, rowCount: 100 });
    ...
    throw new BadRequestException('모든 userNetID에서 데이터를 받아오지 못했습니다.');
  }
  /** 큐 맨 뒤로 보내기 */
  ...
  async getRankingSummary(userIds: string[]) {
    if (!userIds.length) throw new BadRequestException('userNetIDs 파라미터가 비어 있습니다.');
    ...
    // 3. 솔로/트리오 데이터
    const soloRaw = await this.rankingService.getRankingData({ userNetID: validUid, teamMode: 1, region: 'ES', rankingType: 1, champType: 0, rowCount: 100 });
    const trioRaw = await this.rankingService.getRankingData({ userNetID: validUid, teamMode: 2, region: 'ES', rankingType: 1, champType: 0, rowCount: 100 });
    ...
    if (backupData) {
      ...
      soloPlayers = soloNow.map(p => ({ ...p, rank_change: 'new', score_change: null }));
      trioPlayers = trioNow.map(p => ({ ...p, rank_change: 'new', score_change: null }));
    }
    // 5. 캐릭터별 1등 마킹
    ...
    return {
      ...
      now_time: DateTime.now().setZone('Asia/Seoul').toFormat('yyyy-MM-dd HH:mm:ss'),
    };
  }
}
